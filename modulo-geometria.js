...
function Esfera(radio) {
    ...
    this.getPosicion=function(u,v){
        var x = Math.cos(2 * Math.PI * u) * Math.sin(Math.PI * v) * radio;
        var y = Math.sin(2 * Math.PI * u) * Math.sin(Math.PI * v) * radio;
        var z = -Math.cos(Math.PI * v) * radio;
        return [x,y,z];
    }
...
function TuboSenoidal(amplitud, longitud, radio, altura) {
    ...
    this.getPosicion=function(u,v){
        ...
        var x = radio * Math.cos(2*Math.PI*u) + amplitud *Math.cos(v*2*Math.PI/longitud) * Math.cos(2*Math.PI*u);
        var z = radio * Math.sin(2*Math.PI*u) + amplitud *Math.cos(v*2*Math.PI/longitud) * Math.sin(2*Math.PI*u);
        var y = altura / 2 * (v - 1/2);
        return [x,y,z];
    }

    this.getNormal=function(u,v){
        a1 = this.getPosicion(u+0.00001,v);
        a2 = this.getPosicion(u,v+0.00001);

        var s1 = a1 - this.getPosicion(u,v);
        var s2 = a2 - this.getPosicion(u,v);


        var x = s1[1] * s2[2] - s1[2] * s2[1];
        var y = s1[2] * s2[0] - s1[0] * s2[2];
        var z = s1[0] * s2[1] - s1[1] * s2[0];
